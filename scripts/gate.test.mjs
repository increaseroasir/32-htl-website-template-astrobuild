...
/**
 * The browser half of the Lead conversion and its ONE permitted gate
 * (Lead Vault, Job 3). Both defects reintroduced: the gate removed
 * (every repeat customer double-fires) and the gate widened to a sync
 * status (every CRM hiccup silently starves Meta of paid-for signal).
 */
const DUP_GATE = 'Browser conversion gated on duplicate status only';

test('FAILS when the duplicate guard is removed from the island', async () => {
  const run = await withFixture(async ({ dir }) => {
    const file = join(dir, 'src', 'components', 'islands', 'Quiz.tsx');
    const body = await readFile(file, 'utf8');
    await writeFile(file, body.replace('if (!data.duplicate) {', 'if (true) {'));
  });
  assertFails(run, DUP_GATE);
});

test('FAILS when the conversion is gated on a sync status instead', async () => {
  const run = await withFixture(async ({ dir }) => {
    const file = join(dir, 'src', 'components', 'islands', 'Quiz.tsx');
    const body = await readFile(file, 'utf8');
    await writeFile(
      file,
      body.replace('if (!data.duplicate) {', 'if (!data.duplicate && data.ghlOk) {'),
    );
  });
  assertFails(run, DUP_GATE);
});

test('PASSES with the guard exactly !data.duplicate', async () => {
  assertPasses(await withFixture(null), DUP_GATE);
});

/**
 * THE LEAD VAULT SEALS (Job 5). Each of the five checks gets its defect
 * reintroduced, exactly as it would happen in a careless edit.
 */
test('FAILS when a vault write appears before the D1 INSERT', async () => {
  const run = await withFixture(async ({ dir }) => {
    const file = join(dir, 'src', 'pages', 'api', 'lead.ts');
    const body = await readFile(file, 'utf8');
    // The careless edit: "write the sheet first so nothing is lost".
    await writeFile(
      file,
      body.replace(
        'const db = getDb();',
        'const db = getDb();\n  const _sheetFirst = () => upsertRowByLeadUuid(null as never, "", []);',
      ),
    );
  });
  assertFails(run, 'Vault write comes after the D1 write');
});

test('FAILS when the vault task can answer the customer', async () => {
  const run = await withFixture(async ({ dir }) => {
    const file = join(dir, 'src', 'pages', 'api', 'lead.ts');
    const body = await readFile(file, 'utf8');
    // The Sun Pool 503: sheet unconfigured refuses the lead.
    await writeFile(
      file,
      body.replace(
        'const vaultTask = (async () => {',
        'const vaultTask = (async () => {\n      if (!sheetsConfigFromEnv(e)) return json({ ok: false }, 503);',
      ),
    );
  });
  assertFails(run, 'Vault write cannot fail the lead');
});

test('FAILS when the response awaits the vault task', async () => {
  const run = await withFixture(async ({ dir }) => {
    const file = join(dir, 'src', 'pages', 'api', 'lead.ts');
    const body = await readFile(file, 'utf8');
    await writeFile(
...
test('FAILS when the append uses an open column range (the column-AF bug)', async () => {
  const run = await withFixture(async ({ dir }) => {
    const file = join(dir, 'src', 'lib', 'sheets.ts');
    const body = await readFile(file, 'utf8');
    await writeFile(file, body.replace("`'${tab}'!A1`", "`'${tab}'!A:AI`"));
  });
  assertFails(run, 'Sheet append anchors to A1, never an open range');
});

test('FAILS when a Google secret is dropped from the env schema', async () => {
  const run = await withFixture(async ({ dir }) => {
    const file = join(dir, 'src', 'env.d.ts');
    const body = await readFile(file, 'utf8');
    await writeFile(file, body.replace(/^.*GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY.*$/m, ''));
  });
  assertFails(run, 'Google secrets declared in the env schema');
});

test('FAILS when META_TEST_EVENT_CODE is committed to wrangler.toml', async () => {
  const run = await withFixture(async ({ dir }) => {
    const file = join(dir, 'wrangler.toml');
    const body = await readFile(file, 'utf8');
    await writeFile(file, `${body}\n[vars]\nMETA_TEST_EVENT_CODE = "TEST12345"\n`);
  });
  assertFails(run, 'No Meta test event code in committed config');
});

test('PASSES all five vault seals on the clean tree', async () => {
  const run = await withFixture(null);
  assertPasses(run, 'Vault write comes after the D1 write');
  assertPasses(run, 'Vault write cannot fail the lead');
  assertPasses(run, 'Sheet append anchors to A1, never an open range');
  assertPasses(run, 'Google secrets declared in the env schema');
  assertPasses(run, 'No Meta test event code in committed config');
});

/**
 * A founded year in the future (moved out of the schema at the Astro 7
 * upgrade — Workers freeze the clock at 0 in global scope, so the schema
 * ceiling is static and the real check lives in the gate's Node clock).
 */
test('FAILS when foundedYear is in the future', async () => {
  const run = await withFixture(
    null,
    cleanManifest({
      identity: { name: 'Fixture Spas', siteUrl: 'https://fixture-spas.test', foundedYear: 3000 },
    }),
  );
  assertFails(run, 'Founded year is not in the future');
});

test('PASSES on a past foundedYear', async () => {
  assertPasses(await withFixture(null), 'Founded year is not in the future');
});
