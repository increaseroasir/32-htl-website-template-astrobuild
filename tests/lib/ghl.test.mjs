...
test('enrichment adds the Model Interest tag and the product\'s own tags (Job 4)', () => {
  const tags = buildGhlTags({
    category: 'hot-tubs',
    productSlug: 'cascade-x200',
    utmSource: '',
    utmCampaign: '',
    productName: 'Cascade X200',
    productTags: [' Floor-Model ', '', 'Clearance'],
  });
  assert.ok(tags.includes('model interest - cascade x200'), tags.join(','));
  assert.ok(tags.includes('floor-model'), 'admin tags trimmed + lowercased');
  assert.ok(tags.includes('clearance'));
  assert.ok(!tags.includes(''), 'blank admin tags dropped');
  // Without enrichment, no Model Interest tag appears.
  const bare = buildGhlTags({ category: '', productSlug: '', utmSource: '', utmCampaign: '' });
  assert.ok(!bare.some((t) => t.startsWith('model interest')));
});
