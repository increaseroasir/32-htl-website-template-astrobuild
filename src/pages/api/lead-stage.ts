...
export const POST: APIRoute = async ({ request }) => {
  ...
  // A failed upstream mint reaches here dressed as a value: GHL renders
  // an empty merge field as the literal "null" (or "undefined"), and an
  // unresolved template arrives as the raw "{{custom_webhook...}}" text.
  // Those are caller bugs, not unknown leads — answer 400 with the cause
  // named, so the execution log reads "mint failed upstream", never a
  // 404 that looks like a genuinely missing lead. No row is written on
  // any of these paths.
  if (/^(?:null|undefined)$/i.test(leadUuid) || leadUuid.includes('{{')) {
    console.warn('[stage] leadUuid is a merge sentinel — the upstream mint failed or the merge field did not resolve:', leadUuid.slice(0, 40));
    return json(
      { ok: false, error: 'leadUuid is a GHL merge sentinel — the upstream mint failed or the merge field did not resolve.' },
      400,
    );
  }
