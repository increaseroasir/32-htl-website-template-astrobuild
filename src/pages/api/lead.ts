...
export const POST: APIRoute = async ({ request, cookies, clientAddress, locals }) => {
  ...
  if (verdict.kind === 'drop') {
    ...
    return json({
      ...
      // The real success response carries this field; the fake one must
  ...
  // would match itself). Same human inside 24h whose prior conversion
  // COUNTED → suppress both halves of the conversion. Prior FAILED →
  // RETRY: let it fire, recovering the lost signal. The lead itself is
  // stored and synced either way — suppression is about ad signal only.
  let duplicate: DuplicateDecision = { suppress: false, priorUuid: '' };
  try {
    duplicate = duplicateDecision(await findRecentDuplicate(db, { email, phone, now }));
  } catch (error) {
    // The check optimises ad spend; it is not a gatekeeper. A broken
    // duplicate query must never refuse or delay the lead.
    const detail = error instanceof Error ? error.message : String(error);
    console.error('[lead] duplicate check failed — treating as new:', detail);
  }
  if (duplicate.suppress) {
    console.warn(
      `[lead] duplicate within 24h of ${duplicate.priorUuid} — conversion suppressed, lead still stored and synced`,
    );
  }
  ...
  try {
    ...
    // PRODUCT ENRICHMENT (Job 4): the product's own facts, read from D1
    // by slug, attached to the CRM record so the salesperson sees WHAT
    // they want without a second lookup — AND frozen into the vault
    // row's snapshot columns (owner ruling 2026-08-14; D1 stays the
    // source of truth). Never blocks: null = sync unenriched.
    const product = await enrichFromInventory(db, productSlug);
    ...
    const [ghlResult, capiResult] = await Promise.all([
        ...
        ? syncToGhl(
            { apiKey: e.GHL_API_KEY!, locationId: e.GHL_LOCATION_ID! },
            {
              ...
              customFields: {
                ...
                // D1 product facts (Job 4). Strings — GHL custom fields
                // are text; the CRM is a display surface, not a ledger.
                ...(product
                  ? {
                      product_name: product.name,
                      product_id: String(product.id),
                      product_price: String(product.price),
                      product_monthly: String(product.monthlyPayment),
                      product_stock: product.stockStatus,
                    }
                  : {}),
                message,
      ...
      // THE SERVER HALF OF THE CONVERSION IS GATED ON DUPLICATE STATUS
      // AND NOTHING ELSE (the browser half mirrors this — see Quiz.tsx).
      // A GHL outage must not stop the ad signal; a duplicate must.
      metaEnabled && !duplicate.suppress
        ? sendMetaCapi(
            {
        ...
        : Promise.resolve<CapiResult>({
            ok: false,
            status: 0,
            detail: duplicate.suppress ? 'duplicate — conversion suppressed' : 'disabled',
          }),
    ]);
    ...
    if (!capiResult.ok && metaEnabled && !duplicate.suppress) {
    ...
    /* ---------------------------------------------------------------
       THE LEAD VAULT (Google Sheets mirror). THE VAULT IS A PROJECTION
       OF D1: the D1 write already happened above — D1 is the source of
       truth, the sheet is a view of it, rebuildable from D1 at any
       time. The write runs in waitUntil, AFTER the customer's response
       is on the wire, and NOTHING in it can fail the lead: a Google
       outage is a reporting problem, not a lost customer.

       One lead identity, one row, forever — the writer UPSERTS by
       lead_uuid (repeat submission updates the row, never appends a
       second one), resolving the row by id lookup at write time.

       Every outcome lands on the D1 row's vault_status/vault_error
       columns (migration 0003) — SENT / DRIFTED / FAILED /
       UNCONFIGURED — so the retry job (later ticket) has something to
       find. A 429 or 5xx does NOT vanish into a console.error; the
       console line is commentary, the D1 column is the record.
       --------------------------------------------------------------- */
    const vaultTask = (async () => {
      ...
      try {
        ...
        // check, so removing the secrets can never overwrite the DRIFTED
        // mark with UNCONFIGURED and lose the only pointer to the orphan.
        // A drifted row is invisible to the column-A lookup, so an upsert
        // here would append a twin. Mark stays, human repairs the sheet.
        const prior = await db
          .prepare('SELECT vault_status FROM leads WHERE uuid = ?')
          .bind(uuid)
          .first<{ vault_status: string }>();
        if (!shouldWriteVaultRow(prior?.vault_status ?? '')) {
          console.error(
            `[vault] lead ${uuid} is marked DRIFTED — sheet write REFUSED. ` +
              'Drifted is not retryable: repair the sheet layout by hand, fix the D1 mark, then retry.',
          );
        } else if (!sheets) {
          // Not configured is a STATE, not an error — but it is recorded,
          // so "why is the sheet empty?" is answerable from D1.
          await db
          ...
          await writeVaultRow(sheets);
        }
      } catch (error) {
        // The one path where the console IS the record: even the D1
        // status write failed. The lead itself is long since safe.
        const detail = error instanceof Error ? error.message : String(error);
        console.error('[vault] task failed AND could not record itself in D1:', detail);
      }

      /* -------------------------------------------------------------
         FAILURE VISIBILITY (Job 4). A failed leg becomes a row a human
         SEES and an alert a human GETS — never only a console line.
         Spec: GHL failure → Missed Leads row + alert. CAPI failure →
         alert (the expensive one). Vault failure/drift → alert loudly
         (if the durability mirror is down, everything after it is
         unmonitored). Own try/catch: alerting may never fail a lead.
         ------------------------------------------------------------- */
      try {
        if (ghlEnabled && !ghlResult.ok) {
          failures.unshift({ kind: 'ghl', detail: `${ghlResult.status}: ${ghlResult.detail.slice(0, 300)}` });
        }
        if (metaEnabled && !duplicate.suppress && !capiResult.ok) {
          failures.push({ kind: 'capi', detail: `${capiResult.status}: ${capiResult.detail.slice(0, 300)}` });
        }
        if (failures.length === 0) return;

        const ghlFailure = failures.find((f) => f.kind === 'ghl');
        if (ghlFailure && sheets) {
          // The Missed Leads tab is an incident LOG (append-only, one
          // row per failure, never upserted) — the human-scannable list
          // of every lead the CRM did not receive.
          await appendRow(
            sheets,
            buildMissedRow({
              loggedAt: Date.now(),
      ...
      /** The vault write proper — split out so the failure pass above
       *  always runs regardless of which branch the write took. */
      async function writeVaultRow(config: NonNullable<ReturnType<typeof sheetsConfigFromEnv>>) {
        const result = await upsertRowByLeadUuid(
          ...
          buildVaultRow({
            ...
            // sheet is a projection and may never disagree with it.
            capiStatus: conversionStatus,
            capiError: conversionStatus === 'FAILED' ? capiResult.detail.slice(0, 300) : '',
            consentVersion,
            conversionValue: leadValue,
            // Submit-time snapshot for columns 35–40 — same enrichment
            // object the CRM sync used, so sheet and CRM can't disagree.
            product,
          }),
        );
        ...
        if (status !== 'SENT') {
          ...
          // after it is unmonitored).
          failures.push({
            kind: 'vault',
            detail: status === 'DRIFTED' ? `DRIFTED — ${detail}` : detail,
          });
        }
      }
    })();

    // waitUntil keeps the isolate alive past the response. Absent only
    // outside the Workers runtime (plain astro dev) — there the task has
    // already started and its own catch swallows every failure mode.
    if (locals.cfContext?.waitUntil) {
      locals.cfContext.waitUntil(vaultTask);
    }

    // `duplicate` is the ONLY signal the browser may gate its half of
    // the conversion on (gate-enforced). Everything else that went wrong
    // above is a server problem the browser can do nothing about.
    return json({ ok: true, leadUuid: uuid, eventId, duplicate: duplicate.suppress });
  } catch (error) {
